...
export function RoomCard({ room, currentUserId, onJoin, onDeleted }: RoomCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative flex overflow-hidden rounded-2xl bg-unyamo-surface border border-unyamo-border shadow-sm"
    ...
    >
      ...
      <div className="flex-1 p-3">
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <p className="font-heading font-bold text-unyamo-ink text-sm leading-tight truncate max-w-[140px]">
            {room.name}
          </p>
          <div className="flex items-center gap-1 flex-shrink-0">
            {room.isPrivate && (
              <Badge
                variant="outline"
                className="border-unyamo-border text-unyamo-ink-muted text-[10px] px-1.5 py-0"
            ...
            <Badge
              className={cn(
                ...
                isWaiting && !isFull
                  ? 'bg-unyamo-green-bright text-white'
                  : isWaiting && isFull
                    ? 'bg-unyamo-red text-white'
                    : 'bg-unyamo-ink-muted text-white',
              )}
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="text-xs text-unyamo-ink-muted space-y-0.5 min-w-0">
            <p className="truncate">
              <span className="font-medium">HOST</span>
              {' '}
              {room.hostName}
            </p>
            <p className={cn(isFull ? 'text-unyamo-red font-semibold' : '')}>
          ...
          <div className="flex flex-col gap-1 items-end">
            <Button
              ...
              className={cn(
                ...
                isFull || !isWaiting
                  ? 'bg-unyamo-border text-unyamo-ink-muted cursor-not-allowed'
                  : 'bg-unyamo-green text-white hover:bg-unyamo-green/90',
              )}
            ...
            {isHost && (
              <Button
                size="sm"
                variant="destructive"
                className="rounded-full text-[10px] h-6 px-2.5 font-bold bg-unyamo-red hover:bg-unyamo-red/90 text-white"
        ...
        {deleteError && (
          <p className="text-unyamo-red text-xs mt-2" role="alert">{deleteError}</p>
        )}
      </div>
    </motion.div>
  )
