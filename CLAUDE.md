...
- **DRAW フェーズ**: 山札から引く / 捨て札トップから拾う / **ウニャモ宣言**（ドロー前のみ可）
- **DISCARD フェーズ**: 1枚捨てる / 同ランク2〜3枚の特殊操作（1ターン1回）
- **ウニャモ宣言**: 手札合計 5点以下、自ターン開始時、未宣言の場合のみ
...
- `src/game-logic/` — 純粋関数（deck / scoring / turn / unyamo / validation / cpu）。Vitest で網羅
...
- **`judgeWinner` の順位バグ**: 非宣言者が全員 2位になっていた。`src/game-logic/unyamo.ts` で `findIndex` ベースの順位計算に修正済み
- **`*.png` を `.gitignore` で除外しているため `public/icons/*.png` には例外 `!public/icons/*.png` が必要**

## 仕様変更時のチェックリスト

1. `unyamo-specification.md` を更新
