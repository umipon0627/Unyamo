...
オリジナルトランプゲーム「Unyam」のオンライン対戦 Web アプリ。
詳細仕様は [`unyam-specification.md`](./unyam-specification.md) を参照。
...
- **ウニャム宣言**: 自ターン開始時、ドロー前に手札合計が **5点以下** なら宣言可能。同一ゲーム内では一度しか宣言できない
...
- **サーバー側検証**: ターン・フェーズ・カード存在・特殊操作・ウニャム条件・重複操作を `party/game-server.ts` で全て検証
